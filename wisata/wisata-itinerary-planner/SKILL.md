...
- **Setiap entri WAJIB, tanpa kecuali (termasuk Cadangan):**
  ...
  - **Badge ✅/⚠️ di akhir judul HARUS polos** — jangan tambahkan klausa penjelas setelahnya (mis. `⚠️ — koreksi akses`, `⚠️ — dievaluasi atas usulan pengguna`). Wisata.html cuma tahu cara menghapus badge polos dari tampilan; klausa tambahan ikut tampil apa adanya di judul. Taruh catatan/koreksi itu di badan uraian atau `**Kenapa tidak dimasukkan:**`, bukan di judul. Pengecualian: akhiran `— Plan-B Hari N (alasan singkat)` pada entri berlabel 🏷️ (lihat bawah) itu konvensi tersendiri, bukan klausa liar.
